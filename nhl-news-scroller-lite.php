<?php
/*
Plugin Name: NHL Hockey News Scroller Lite
Description: Provides the Latest NHL Headlines, updated every 3-4 hours
Author: A93D
Version: 0.8.1
...
function NHLSLLPageURL() {
 $NHLSLLpageURL = 'http';
 $NHLSLLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $NHLSLLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $NHLSLLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $NHLSLLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_nhlslstats() 
{
?>
<h2>NHL Latest News Lite</h2>
<?php nhlsl_stats(); ?>
<?php
}

function nhlslstats_install()
{
register_sidebar_widget(__('NHL News Scroll Lite'), 'widget_nhlslstats'); 
}
add_action("plugins_loaded", "nhlslstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'nhlsl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'nhlsl_stats_remove' );

function nhlsl_stats_install() 
{
...
add_option("nhlsl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("nhlsl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("nhlsl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE NHL SCROLLER Installation", "Hi\n\nLITE NHL SCROLLER Activated at \n\n".NHLSLLPageURL()."\n\nNHL SCROLLER Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function nhlsl_stats_remove() 
{
/* Deletes the database field */
delete_option('nhlsl_scroll_text_color');
delete_option('nhlsl_scroll_text_color1');
delete_option('nhlsl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'nhlsl_stats_admin_menu');

function nhlsl_stats_admin_menu() {

add_options_page('NHL Scroller Lite', 'NHL Scroller Lite Settings', 'administrator', 'nhl-news-scroller-lite.php', 'nhlsl_stats_plugin_page');
}
}
function nhlsl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('nhlsl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('nhlsl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('nhlsl_scroll_text_color2'));

$mydisplay = "http://www.ibet.ws/nhl_news_scroll_lite/nhl_news_scroll_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $mydisplay;
$rss = nhlsl_fetch_rss( $url );
...
function nhlsl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>NHL Scroller Lite Options Page</h2>
...
<!-- Start Advanced Plugins List -->
  ...
  <h5>Step 1. <?php _e('Use the link below to upgrade to our FREE advanced NHL News Scroller package') ?></h5>
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/nhl-news-scroller.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/nhl-news-scroller.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
